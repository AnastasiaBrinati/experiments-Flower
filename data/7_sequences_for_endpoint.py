...
def create_and_save_sequences(endpoint_id, train_path, test_path):
    ...
    output_features = ["timestamp", "avg_total_execution_time", "avg_system_processing_time"]
    ...
    dataset_dict.push_to_hub("anastasiafrosted/sequenced_endpoint_"+endpoint_id)
...
if __name__ == "__main__":
    for i in range(6,10):
        train_path = "scaled_data/globus/endpoints/endpoint"+str(i)+"/split/train/train.csv"
        test_path = "scaled_data/globus/endpoints/endpoint"+str(i)+"/split/test/test.csv"
        create_and_save_sequences(str(i), train_path, test_path)
